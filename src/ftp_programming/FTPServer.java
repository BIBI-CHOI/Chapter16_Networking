...
/** [파일 서비스 프로그래밍]
 * 1. 클라이언트가 특정한 파일명을 서버에 요청한다.
 * 2. 서버는 클라이언트가 요청한 파일을 찾는다.
 * 3. 요청한 파일을 찾으면 파일(리소스폴더 내)에서 데이터를 읽어 들인다.
 * 4. 읽어 온 파일의 데이터를 클라이언트에게 전송한다.
 * 5. 클라이언트는 받은 파일의 내용을 화면에 출력하고 저장한다. 
 */
public class FTPServer {
	private static final int PORT = 56486;
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(timeStamp() + clientAddr + "<-coonected.");
